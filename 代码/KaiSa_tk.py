...
from KaiSa import KaiSa_cipher
def create_KaiSa_interface(app):
    def encrypt_text():
        ...
        try:
            ...
            encrypted = KaiSa_cipher(input_text, shift)
            ...
            entry_result.delete("1.0", tk.END)
            entry_result.insert("1.0", "密钥格式错误")
    ...
    def decrypt_text():
        ...
        try:
            ...
            shift = int(entry_shift.get())
            decrypted = KaiSa_cipher(input_text, -shift)
            entry_result.delete("1.0", tk.END)
    ...
    # 密钥
    tk.Label(app, text="密钥（数字）:").grid(row=2, column=0, sticky='w')
    entry_shift = tk.Entry(app)
    entry_shift.grid(row=2, column=1, sticky='w')
