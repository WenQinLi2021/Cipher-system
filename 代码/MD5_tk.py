...
def create_MD5_interface(app):
    def md5_encrypt(input_text):
        ...
        md5_obj = hashlib.md5()
        md5_obj.update(input_text.encode('utf-8'))
    ...
    def encrypt_file():
        ...
        if file_path:
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                # 加密并显示结果
                encrypted_text = md5_encrypt(data)
                txt_result.delete("1.0", tk.END)
                txt_result.insert(tk.END, encrypted_text)
    ...
    # 结果显示框
    tk.Label(app, text="MD5 加密结果:").grid(row=2, column=0, sticky='w')
